...
\end{wrapfigure}
Donc une List d'entier se définit comme: <List <Int> >. Une chose importante à remarquer est que j'ai utiliser le mot "représentation" en effet <Int> n'est pas un entier mais une représentation d'entier.\\
...
\end{lstlisting}
Notre "proc {Q}" sera stocker comme: "q = (proc\{\$\}\{P\} end, \{$P\rightarrow p$\})". On lit donc, la procédure \textit{anonyme} (\$), fais un appel à P (\{P\}) et fini (end), son \textit{environnement contextuel} fait que lorsqu'on appelle "P" on va récupérer la valeur "p" en mémoire (\{$P\rightarrow p$\}). Donc on voit que l'\textit{environnement contextuel} est stocké avec le code de procédure.\\
...
\item \textcolor{red}{$\beta$-renaming}: applique une fonction à un argument
...
Quand on manipule des expression lambda, il est important de suivre quelques règles:µ
\begin{itemize}
...
\item On étend vers la droit. ex: $\lambda x.t_1 t_2 \rightarrow \lambda x.(t_1 t_2)$
...
\subsection{Nombres}
On utilise une notation appelé \textcolor{red}{Church Numerals}:
\begin{itemize}
...
\end{itemize}
On peut remarquer que ces fonctions dont d'ordre supérieur. En effet, il prend en argument une fonction et ressort une fonction. (d'un seul argument)\\
...
\begin{align*}
AND &\triangleq \lambda p.\lambda q.p \quad q \quad p & OR &\triangleq \lambda p.\lambda q.p \quad p \quad p\\
...
La \textit{programmation fonctionnel} peut être compris en terme de lambda calcul.
\begin{itemize}
\item Les valeurs de procédures (donc \textit{lexically scope closures}) \textbf{sont} des fonctions lambda.
\item l'\textit{Eager et lazy evaluation} est une juste une stratégie de réduction différente.
\end{itemize}
...
\subsection{Astuce pour le Lambda Calcul}
Voici quelques astuces qui peuvent vous aider à maitriser le lambda calcul qui peut semble de prime abord barbare:
\begin{enumerate}
\item Ajouter des parenthèse !! Ex: $\lambda x.\lambda y.x + y \quad 1 \quad 2 \rightarrow (\lambda x.(\lambda y.x+y)1)2$ Cela peut sembler anodin mais cela permet de mieux comprendre et lire les expression qui peuvent vite devenir lourdes.
\item Utiliser des abréviations. Car on a pas toujours besoin de savoir comment cela fonctionne et permet de faire partie par partie une fonction.
\end{enumerate}
