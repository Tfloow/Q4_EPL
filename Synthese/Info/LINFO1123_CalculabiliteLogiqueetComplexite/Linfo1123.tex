...
\subsection{Composition de la logique}
\begin{center}
...
\item \textit{Connecteur logique}: ne pas $\neg$, conjonction $\wedge$, disjunction $\vee$, implication $\Rightarrow$ et équivalence $\Leftrightarrow$
...
\begin{tabular}{|cc|cc|cc|}
\hline
$\neg (\neq p)$ & $p$ & $p \wedge q$ & $q \wedge p$ & $p \vee q$ &  $q \vee p$\\
\hline
...
Mais, on a le \textcolor{red}{problème de la satisfiabilité} comment savoir si une formule F a un modèle. Le modèle est une "\textit{solution}" qui rend cette formule F.\\
...
\item Pour chaque noeuds $v \in V$, la disjonction des coleur $ [v,c] $ sur toutes les couleurs $c \in C$ nous assure que chaque noeud a au moins une couleur
\item Pour chaque noeud $v \in V$ a conjonction va s'assurer que chaque noeud n'a pas plus d'une couleur: $\neg ( \text{couleur}[v, c_1] \wedge \text{couleur}[v, c_2]$ on utilise des paires de couleurs $ [c_1, c_2] $
\end{itemize}
...
\end{enumerate}
On peu réduire le 1. au 2. via: $p \models q ssi (p \wedge \neg q) \notin SAT$\\
...
\item Ajouteer la clause conclusion à la formule CNF
\end{enumerate}
